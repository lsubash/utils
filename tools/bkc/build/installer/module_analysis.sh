...
# specified in the environment. ### rksavino: this section no longer needed because it runs on tagent start only now
#SAVED_MEASUREMENT=/opt/trustagent/var/measureLog.xml
mkdir -p /opt/bkc-tool/var/system-info
mkdir -p /opt/bkc-tool/var/ramfs
mkdir -p /var/log/bkc-tool
...
# remove any old module log output
if [ -f "${OUTFILE}" ]; then
  rm -f "${OUTFILE}"
fi
...
INFILE_TCB_MEASUREMENT_SHA256=${INFILE_TCB_MEASUREMENT_SHA256:-/var/log/bkc-tool/measurement.bin}
...
#Run module_analysis_da_tcg.sh if TCG formatted event log found
tcgEventLogPresent="$(get_line_number 'TCG Event Log')"
if [ "$tcgEventLogPresent" != 0 ]; then
  `echo $0 | sed "s/\.sh/_da_tcg\.sh/g"` $1
  exit 0
fi

 #Run module_analysis_da.sh if TPM version is 2.0 and "Type: 0x401" does not exist due to ACM update
 num="`get_line_number 'TPM2.0'`"
 for parm in $num;do
 if [ $parm -gt 0 ];then
     `echo $0 | sed "s/\.sh/_da\.sh/g"` $1
     exit 0
...
if [ $txt_status -eq 2 -a $x501Data!="EOF" ];then
  ...
  do
    ...
    if [ $mrefs_count -eq 1 ] || [ $mrefs_count -eq 2 ]; then
      case $index in
        "17")
        ...
        "18")
          # 18 contains vmlinuz
          measurement_name="vmlinuz"
          ;;
        "19")
          # 19 contains initrd
          measurement_name="initrd"
          ;;
        "22")
